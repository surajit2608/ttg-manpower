        ...
        <div class="col-xs-12 col-sm-12 col-md-6">
          <div class="group">
            <label class="d-flex justify-content-space-between align-items-center">
              ...
              {{#if index > 0}}
                <a class="color-danger" on-click="onPressRemoveTraining" class-disabled="{{trainings[0].no_training}}"><i class="icon-trash"></i></a>
              {{/if}}
            </label>
...
<script type="text/javascript">
  ...
  $Event.on('page.init', () => {
    if ($Data.get('tab') == 'trainings') {
      ...
      $Api.get('<%SITE_URL%>/api/applications/trainings/get').params(params).send((res) => {
        if (!$Data.get('trainings[0].id')) {
          $Data.set('trainings[0].no_training', 0)
          $Data.set('trainings[0].worker_id', $Data.get('me.id'))
        }
      })
    }
  })
  ...
  $Event.on('onPressSaveTrainings', (callback) => {
    var params = $Data.get('trainings')
    $Api.post('<%SITE_URL%>/api/applications/trainings/save').params(params).send(res => {
      callback(res)
    })
  })
</script>
