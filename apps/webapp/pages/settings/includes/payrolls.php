...
<script type="text/javascript">
  ...
  $Event.on('page.init', () => {
    if ($Data.get('tab') == 'payrolls') {
      ...
      $Api.get('<%SITE_URL%>/api/applications/payrolls/get').params(params).send((res) => {
        if (!$Data.get('payrolls[0].id')) {
          $Data.set('payrolls[0].worker_id', $Data.get('me.id'))
        }
      })
    }
  })
  ...
  $Event.on('onPressSavePayrolls', (callback) => {
    var params = $Data.get('payrolls')
    $Api.post('<%SITE_URL%>/api/applications/payrolls/save').params(params).send(res => {
      callback(res)
    })
  })
</script>
