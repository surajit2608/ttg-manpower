...
<script type="text/javascript">
  ...
  $Event.on('page.init', () => {
    if ($Data.get('tab') == 'addresses') {
      ...
      $Api.get('<%SITE_URL%>/api/applications/addresses/get').params(params).send((res) => {
        if (!$Data.get('addresses[0].id')) {
          $Data.set('addresses[0].worker_id', $Data.get('me.id'))
        }
      })
    }
  })
  ...
  $Event.on('onPressSaveAddresses', (callback) => {
    var params = $Data.get('addresses')
    $Api.post('<%SITE_URL%>/api/applications/addresses/save').params(params).send(res => {
      callback(res)
    })
  })
</script>
