...
<script type="text/javascript">
  ...
  $Event.on('page.init', () => {
    if ($Data.get('tab') == 'health') {
      ...
      $Api.get('<%SITE_URL%>/api/applications/health/get').params(params).send((res) => {
        if (!$Data.get('health.id')) {
          $Data.set('health.worker_id', $Data.get('me.id'))
        }
      })
    }
  })
  ...
  $Event.on('onPressSaveHealth', (callback) => {
    var params = $Data.get('health')
    $Api.post('<%SITE_URL%>/api/applications/health/save').params(params).send(res => {
      callback(res)
    })
  })
</script>
