<div class="col-xs-12 col-sm-12 col-md-12">
  <div class="d-flex justify-content-space-between align-items-center mb-1">
    <div>
      <div class="d-flex">
        <input class="controls ph-0_5 pv-0_25 round-tr-0 round-br-0" value="{{search}}" placeholder="Search..." on-enter="onLoadRelationships" />
        {{#if search}}
          <button class="btn p-0_5 round-0 border-1" on-click="onPressClearSearch">
            <i class="icon-close"></i>
          </button>
        {{/if}}
        <button class="btn p-0_5 round-tl-0 round-bl-0 border-1" on-click="onLoadRelationships">
    ...
    <div>
      {{#if me.permission.settings_module.create == 'Allow'}}
        <a class="btn primary small round-0_25" on-click="modal.open" target="add-relationship-modal">
          <i class="icon-plus"></i><span class="ml-0_5 hide-xs">Add Relationship</span>
        </a>
      {{/if}}
    </div>
  </div>
  ...
  <div class="table-responsive">
    <table class="table bordered striped border-0">
      <thead>
        <tr>
          <th width="100px">
            <sorting order="{{order}}" column="id" on-click="onLoadRelationships">ID</sorting>
          </th>
          <th>
            <sorting order="{{order}}" column="name" on-click="onLoadRelationships">Relationship</sorting>
          </th>
            ...
            <tr>
              ...
              <td class="t-align-center ws-nowrap">
                {{#if me.permission.settings_module.update == 'Allow'}}
                  <a class="btn primary d-inline small round-0_25" on-click="onEditRelationship" tooltip="Edit">
                    <i class="icon-edit"></i>
                  </a>
                {{/if}}
                {{#if me.permission.settings_module.remove == 'Allow'}}
                  <a class="btn danger d-inline small round-0_25" on-click="onDeleteRelationship" tooltip="Delete">
  ...
  {{#if relationships_loaded && relationships.length}}
...
<script type="text/javascript">
  ...
  $Data.set('$url', '<%ADMIN_URL%>/api/relationships/all')

  $Event.on('page.init', () => {
    if ($Data.get('tab') == 'relationship') {
      $Event.fire('onLoadRelationships')
    }
  })

  $Event.on('onLoadRelationships', () => {
    var params = {
      ...$Data.get('order'),
      ...$Data.get('pagination'),
      search: $Data.get('search')
    }
    $Api.get('<%ADMIN_URL%>/api/relationships/all').params(params).send()
  })
  ...
  $Event.on('onEditRelationship', (e) => {
    $Data.set('relationship', e.get())
    $Event.fire('modal.open', 'add-relationship-modal')
  })

  $Event.on('onDeleteRelationship', (e) => {
    $Data.set('relationship', e.get())
    $Event.fire('modal.open', 'delete-relationship-modal')
  })
</script>
@endsection