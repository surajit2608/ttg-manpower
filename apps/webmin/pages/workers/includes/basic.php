...
<div class="row">
  <div class="col-xs-12 col-sm-6 col-md-4">
    <div class="group">
      ...
      <input type="text" class="controls" value="{{worker.first_name}}" />
    </div>
  </div>
  <div class="col-xs-12 col-sm-6 col-md-4">
    <div class="group">
      <label>Middle Name:</label>
      <input type="text" class="controls" value="{{worker.middle_name}}" />
    </div>
  </div>
  <div class="col-xs-12 col-sm-6 col-md-4">
    <div class="group">
      <label>Last Name<small class="required">*</small>:</label>
      <input type="text" class="controls" value="{{worker.last_name}}" />
    </div>
  </div>
</div>
<div class="row">
...
<div class="row">
  <div class="col-xs-12 col-sm-6 col-md-4">
    <div class="group">
      ...
      <div class="d-flex">
        <fancy-upload class="flex-1 m-0" value="{{worker.image}}" folder="applications/dps" filename="dp" exts="image/png,image/jpeg,image/gif" size="5">
          <button class="btn primary outline block p-0_75 border-1">Browse Photo</button>
        </fancy-upload>
        {{#if worker.image}}<a target="_blank" class="btn primary p-0_75 ml-1" href="<%FULL_URL%>{{worker.image}}"><i class="icon-link"></i></a>{{/if}}
      </div>
    </div>
  </div>
  <div class="col-xs-12 col-sm-6 col-md-4">
...
<div class="row">
  ...
  <div class="col-xs-12 col-sm-6 col-md-4">
    <div class="group">
      ...
      <input type="email" class="controls" value="{{worker.email}}" />
    </div>
  </div>
  <div class="col-xs-12 col-sm-6 col-md-4">
    <div class="group">
      <label>Username<small class="required">*</small>:</label>
      <input type="text" class="controls" value="{{worker.username}}" />
    </div>
  </div>
</div>
<div class="row">
  <div class="col-xs-12 col-sm-6 col-md-4">
    <div class="group">
      ...
      <input type="text" class="controls" value="{{worker.phone}}" />
    </div>
  </div>
  <div class="col-xs-12 col-sm-6 col-md-4">
    <div class="group">
...
<div class="row">
  <div class="col-xs-12">
    <h2 class="mb-1 mt-1 f-size-1_15 f-weight-500 color-danger"><u>Office use only</u>:</h2>
  </div>
  <div class="col-xs-12 col-sm-12 col-md-4">
    <div class="group">
      <label>Registration Date:</label>
      <dropdown-date value="{{worker.registration_date}}" max="{{today}}" />
    </div>
  </div>
  <div class="col-xs-12 col-sm-12 col-md-4">
    <div class="group">
      <label>Job Applied For:</label>
      <dropdown options="{{options_applications}}" value="{{worker.application_id}}" placeholder="Choose Job Applied For" />
    </div>
  </div>
</div>
<div class="row">
  <div class="col-xs-12 col-sm-6 col-md-4">
    <div class="group">
      <label>Joining Date:<small class="ml-0_5">When the worker start working</small></label>
      <dropdown-date value="{{basic.joining_date}}" />
    </div>
  </div>
  <div class="col-xs-12 col-sm-6 col-md-4">
    <div class="group">
      <label>Client Start Date:</label>
      <dropdown-date value="{{basic.client_start_date}}" />
    </div>
  </div>
  <div class="col-xs-12 col-sm-6 col-md-4">
    <div class="group">
      <label>Client End Date:</label>
      <dropdown-date value="{{basic.client_end_date}}" />
    </div>
  </div>
</div>
<div class="row">
  <div class="col-xs-12 col-sm-6 col-md-4">
    <div class="group">
      <label>Holidays Entitlement:</label>
      <input-number value="{{basic.holidays_entitlement}}" />
    </div>
  </div>
</div>
...
<script type="text/javascript">
  ...
  $Event.on('page.init', () => {
    if ($Data.get('tab') == 'basic') {
      ...
      $Api.get('<%ADMIN_URL%>/api/nationalities/options').send()
      $Api.get('<%ADMIN_URL%>/api/relationships/options').send()

      var params = {
        worker_id: $Data.get('worker.id'),
      }
      $Api.get('<%ADMIN_URL%>/api/applications/basic/get').params(params).send((res) => {
        if (!$Data.get('basic.worker_id')) {
          $Data.set('basic.worker_id', $Data.get('worker.id'))
        }
        if (!$Data.get('class.worker_id')) {
          $Data.set('class.worker_id', $Data.get('worker.id'))
        }
        
        $Data.set('change_log.method', 'update')
        $Data.set('change_log.type', 'worker.basic')
        ...
        if (!$Data.get('availability.worker_id')) {
          $Data.set('availability.worker_id', $Data.get('worker.id'))
        }
      })
    }
  })
  ...
  $Event.on('onPressSaveBasic', (callback) => {
    ...
    $Api.post('<%ADMIN_URL%>/api/applications/basic/save').params(params).send(res => {
      callback(res)
    })
  })
