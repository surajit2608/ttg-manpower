...
<script type="text/javascript">
  ...
  $Event.on('page.init', () => {
    if ($Data.get('tab') == 'employments') {
      ...
      $Api.get('<%ADMIN_URL%>/api/applications/employments/get').params(params).send((res) => {
        if ($Data.get('employment')) {
          $Data.set('need_reference', true)
        }
        if (!$Data.get('employments[0].id')) {
          $Data.set('employments[0].no_employment', 0)
          $Data.set('employments[0].worker_id', $Data.get('worker.id'))
        }

        $Data.set('change_log.method', 'update')
        $Data.set('change_log.type', 'worker.employments')
  ...
  $Event.on('onPressSaveEmployments', (callback) => {
    ...
    $Api.post('<%ADMIN_URL%>/api/applications/employments/save').params(params).send(res => {
      callback(res)
    })
  })
</script>
