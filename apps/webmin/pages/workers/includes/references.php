...
<script type="text/javascript">
  ...
  $Event.on('page.init', () => {
    if ($Data.get('tab') == 'references') {
      ...
      $Api.get('<%ADMIN_URL%>/api/applications/references/get').params(params).send((res) => {
        if (!$Data.get('references[0].id')) {
          $Data.set('references[0].worker_id', $Data.get('worker.id'))
          $Data.set('references[1].worker_id', $Data.get('worker.id'))
        }

        $Data.set('change_log.method', 'update')
        $Data.set('change_log.type', 'worker.references')
  ...
  $Event.on('onPressSaveReferences', (callback) => {
    ...
    $Api.post('<%ADMIN_URL%>/api/applications/references/save').params(params).send(res => {
      callback(res)
    })
  })
</script>
