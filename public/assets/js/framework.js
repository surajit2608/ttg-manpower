(function (global, factory) {
  ...
  ((function() { var current = global.$View; var next = factory(); next.noConflict = function() { global.$View = current; return next; }; return global.$View = next; })());
  ...
  if ( !svg ) {
  	createElement = function ( type, ns, extend ) {
  		if ( ns && ns !== html ) {
  			throw 'This browser does not support namespaces other than http://www.w3.org/1999/xhtml. The most likely cause of this error is that you\'re trying to render SVG in an older browser. See http://docs._viewjs.org/latest/svg-and-older-browsers for more information';
  		}
  ...
  if ( hasConsole ) {
  	...
  	var welcomeMessage = "";
  	welcome = function () {
  		if ( $View.WELCOME_MESSAGE === false ) {
  			welcome = noop;
  			return;
  		}
  		var message = 'WELCOME_MESSAGE' in $View ? $View.WELCOME_MESSAGE : welcomeMessage;
  	...
  	printWarning = function ( message, args ) {
  		...
  		if ( typeof args[ args.length - 1 ] === 'object' ) {
  			...
  			var _view = options ? options._view : null;

  			if ( _view ) {
  				// if this is an instance of a component that we know the name of, add
  				// it to the message
  				var name;
  				if ( _view.component && ( name = _view.component.name ) ) {
  					message = "<" + name + "> " + message;
  				}

  				var node;
  				if ( node = ( options.node || ( _view.fragment && _view.fragment.rendered && _view.find( '*' ) ) ) ) {
  					args.push( node );
  				}
  			}
  		}

  		console.warn.apply( console, [ '%cDebuhg Info: %c' + message, 'color: rgb(114, 157, 52);', 'color: rgb(85, 85, 85);' ].concat( args ) );
  	};
  ...
  var missingPlugin = function ( name, type ) { return ("Missing \"" + name + "\" " + type + " plugin. You may need to download a plugin via http://docs._viewjs.org/latest/plugins#" + type + "s"); };

  function findInViewHierarchy ( registryName, _view, name ) {
  	var instance = findInstance( registryName, _view, name );
  	return instance ? instance[ registryName ][ name ] : null;
  }

  function findInstance ( registryName, _view, name ) {
  	while ( _view ) {
  		if ( name in _view[ registryName ] ) {
  			return _view;
  		}
  ...
  function interpolate ( from, to, _view, type ) {
  	if ( from === to ) return null;

  	if ( type ) {
  		var interpol = findInViewHierarchy( 'interpolators', _view, type );
  ...
  var deprecations = {
  	...
  	render: {
  		...
  		message: 'The "init" method has been deprecated ' +
  			...
  			'only once prior to, and regardless of, any eventual _view ' +
  			'instance being rendered, or if you need to access the ' +
  			'rendered DOM, use "onrender" instead. ' +
  			'See http://docs._viewjs.org/latest/migrating for more information.'
  	},
  	complete: {
  		deprecated: 'complete',
  ...
  Hook.prototype.call = function call ( method, _view, arg ) {
  	if ( _view[ method ] ) {
  		arg ? _view[ method ]( arg ) : _view[ method ]();
  		return true;
  	}
  };

  Hook.prototype.fire = function fire ( _view, arg ) {
  	this.call( this.method, _view, arg );

  	// handle deprecations
  	if ( !_view[ this.method ] && this.deprecate && this.call( this.deprecate.deprecated, _view, arg ) ) {
  		if ( this.deprecate.message ) {
  			warnIfDebug( this.deprecate.message );
  		} else {
  			warnIfDebug( 'The method "%s" has been deprecated in favor of "%s" and will likely be removed in a future release. See http://docs._viewjs.org/latest/migrating for more information.', this.deprecate.deprecated, this.deprecate.replacement );
  		}
  	}

  	// TODO should probably use internal method, in case _view.fire was overwritten
  	arg ? _view.fire( this.event, arg ) : _view.fire( this.event );
  };
  ...
  var runloop = {
  	...
  	addInstance: function ( instance ) {
  		if ( batch ) addToArray( batch._views, instance );
  	},
  ...
  function flushChanges () {
  	...
  	while ( i-- ) {
  		...
  		var _view = fragment._view;
  		if ( Object.keys( _view.viewmodel.changes ).length ) {
  			changeHook.fire( _view, _view.viewmodel.changes );
  		}
  		_view.viewmodel.changes = {};
  		removeFromArray( _views, _view );
  	...
  	i = _views.length;
  	while ( i-- ) {
  		var _view$1 = _views[i];
  		changeHook.fire( _view$1, _view$1.viewmodel.changes );
  		_view$1.viewmodel.changes = {};
  	}
  	...
  	if ( batch.fragments.length || batch.immediateObservers.length || batch.deferredObservers.length || batch._views.length || batch.tasks.length ) return flushChanges();
  }
  ...
  function set ( _view, pairs ) {
  	var promise = runloop.start( _view, true );

  	var i = pairs.length;
  	while ( i-- ) {
  		var ref = pairs[i], model = ref[0], value = ref[1];
  		if ( typeof value === 'function' ) value = bind( value, _view );
  		model.set( value );
  	}
  ...
  var star = /\*/;
  function gather ( _view, keypath, base ) {
  	if ( base === void 0 ) base = _view.viewmodel;
  ...
  function build ( _view, keypath, value ) {
  	var sets = [];
  	...
  	if ( isObject( keypath ) ) {
  		var loop = function ( k ) {
  			if ( keypath.hasOwnProperty( k ) ) {
  				sets.push.apply( sets, gather( _view, k ).map( function ( m ) { return [ m, keypath[k] ]; } ) );
  			}
  		};
  	...
  	// set a single keypath
  		...
  		sets.push.apply( sets, gather( _view, keypath ).map( function ( m ) { return [ m, value ]; } ) );
  	}
  ...
  function add ( _view, keypath, d ) {
  	if ( typeof keypath !== 'string' || !isNumeric( d ) ) {
  		throw new Error( 'Bad arguments' );
  	}

  	var sets = build( _view, keypath, d );

  	return set( _view, sets.map( function ( pair ) {
  ...
  function protoAnimate ( _view, model, to, options ) {
  	options = getOptions( options, _view );
  	var from = model.get();
  	...
  	var interpolator = interpolate( from, to, _view, options.interpolator );
  ...
  function $View$animate ( keypath, to, options ) {
  	if ( typeof keypath === 'object' ) {
  		var keys = Object.keys( keypath );

  		throw new Error( ("_view.animate(...) no longer supports objects. Instead of _view.animate({\n  " + (keys.map( function ( key ) { return ("'" + key + "': " + (keypath[ key ])); } ).join( '\n  ' )) + "\n}, {...}), do\n\n" + (keys.map( function ( key ) { return ("_view.animate('" + key + "', " + (keypath[ key ]) + ", {...});"); } ).join( '\n' )) + "\n") );
  	}
  ...
  function $View$detach () {
  	...
  	if ( this.el ) {
  		removeFromArray( this.el.___view_instances__, this );
  	}
  ...
  function $View$find ( selector ) {
  	if ( !this.el ) throw new Error( ("Cannot call _view.find('" + selector + "') unless instance is rendered to the DOM") );
  ...
  function sortByItemPosition ( a, b ) {
  	var ancestryA = getAncestry( a.component || a.__view.proxy );
  	var ancestryB = getAncestry( b.component || b.__view.proxy );
  	...
  	throw new Error( 'An unexpected condition was met while comparing the position of two components. Please file an issue at https://github.com/_viewjs/_view/issues - thanks!' );
  }
  ...
  var Query = function Query ( _view, selector, live, isComponentQuery ) {
  	this._view = _view;
  ...
  function $View$findAll ( selector, options ) {
  	if ( !this.el ) throw new Error( ("Cannot call _view.findAll('" + selector + "', ...) unless instance is rendered to the DOM") );
  	...
  	// Add this to the list of live queries $View needs to maintain,
  	// if applicable
  	if ( query.live ) {
  ...
  function $View$findAllComponents ( selector, options ) {
  	options = options || {};
  ...
  function $View$findComponent ( selector ) {
  	return this.fragment.findComponent( selector );
  }

  function $View$findContainer ( selector ) {
  	if ( this.container ) {
  ...
  function $View$findParent ( selector ) {
  ...
  function enqueue ( _view, event ) {
  	if ( _view.event ) {
  		_view._eventQueue.push( _view.event );
  	}

  	_view.event = event;
  }

  function dequeue ( _view ) {
  	if ( _view._eventQueue.length ) {
  		_view.event = _view._eventQueue.pop();
  	} else {
  		_view.event = null;
  	}
  }
  ...
  // These are then checked against the dependants map (_view.viewmodel.depsMap)
  ...
  function fireEvent ( _view, eventName, options ) {
  	...
  	return fireEventAs( _view, eventNames, options.event, options.args, true );
  }
  ...
  function fireEventAs  ( _view, eventNames, event, args, initialFire ) {
  	...
  	for ( i = eventNames.length; i >= 0; i-- ) {
  		subscribers = _view._subs[ eventNames[ i ] ];

  		if ( subscribers ) {
  			bubble = notifySubscribers( _view, subscribers, event, args ) && bubble;
  		}
  	}

  	dequeue( _view );

  	if ( _view.parent && bubble ) {

  		if ( initialFire && _view.component ) {
  			var fullName = _view.component.name + '.' + eventNames[ eventNames.length-1 ];
  			eventNames = getWildcardNames( fullName );

  			if( event && !event.component ) {
  				event.component = _view;
  			}
  		}

  		fireEventAs( _view.parent, eventNames, event, args );
  	}

  	return bubble;
  }

  function notifySubscribers ( _view, subscribers, event, args ) {
  	...
  	for ( var i = 0, len = subscribers.length; i < len; i += 1 ) {
  		if ( !subscribers[ i ].off && subscribers[ i ].apply( _view, args ) === false ) {
  			stopEvent = true;
  		}
  	}
  ...
  function resolveAmbiguousReference ( fragment, ref ) {
  	...
  	while ( fragment ) {
  		...
  		if ( fragment.context ) {
  			...
  			if ( !fragment.isRoot || fragment._view.component ) hasContextChain = true;
  		...
  		if ( fragment.componentParent && !fragment._view.isolated ) {
  ...
  var KeypathModel = function KeypathModel ( parent, _view ) {
  	this.parent = parent;
  	this._view = _view;
  	this.value = _view ? parent.getKeypath( _view ) : parent.getKeypath();
  	this.deps = [];
  	this.children = {};
  ...
  KeypathModel.prototype.getChild = function getChild ( _view ) {
  	if ( !( _view._guid in this.children ) ) {
  		var model = new KeypathModel( this.parent, _view );
  		this.children[ _view._guid ] = model;
  		model.owner = this;
  	}
  	return this.children[ _view._guid ];
  };
  ...
  KeypathModel.prototype.rebindChildren = function rebindChildren ( next ) {
  	...
  	while ( i-- ) {
  		...
  		child.value = next.getKeypath( child._view );
  		child.handleChange();
  	}
  };
  ...
  KeypathModel.prototype.rebinding = function rebinding ( next, previous ) {
  	var this$1 = this;
  		...
  		var model = next ? next.getKeypathModel( this._view ) : undefined;
  ...
  KeypathModel.prototype.removeChild = function removeChild( model ) {
  	if ( model._view ) delete this.children[ model._view._guid ];
  };
  ...
  ModelBase.prototype.getKeypath = function getKeypath ( _view ) {
  	if ( _view !== this._view && this._link ) return this._link.target.getKeypath( _view );

  	if ( !this.keypath ) {
  		this.keypath = this.parent.isRoot ? this.key : ("" + (this.parent.getKeypath( _view )) + "." + (escapeKey( this.key )));
  	}
  ...
  ModelBase.prototype.registerChange = function registerChange ( key, value ) {
  	if ( !this.isRoot ) {
  		...
  		runloop.addInstance( this.root._view );
  	}
  };
  ...
  var LinkModel = (function (ModelBase) {
  	...
  	LinkModel.prototype.getKeypath = function getKeypath ( _view ) {
  		if ( _view && _view !== this.root._view ) return this.target.getKeypath( _view );

  		return ModelBase.prototype.getKeypath.call( this, _view );
  	};

  	LinkModel.prototype.getKeypathModel = function getKeypathModel ( _view ) {
  		if ( !this.keypathModel ) this.keypathModel = new KeypathModel( this );
  		if ( _view && _view !== this.root._view ) return this.keypathModel.getChild( _view );
  ...
  var Model = (function (ModelBase) {
  	...
  	Model.prototype.adapt = function adapt () {
  		...
  		for ( i = 0; i < len; i += 1 ) {
  			...
  			if ( adaptor.filter( value, keypath, _view ) ) {
  				this$1.wrapper = adaptor.wrap( _view, value, keypath, getPrefixer( keypath ) );
  	...
  	Model.prototype.getKeypathModel = function getKeypathModel ( _view ) {
  		if ( !this.keypathModel ) this.keypathModel = new KeypathModel( this );
  		return this.keypathModel;
  	};
  ...
  function resolveReference ( fragment, ref ) {
  	...
  	if ( ref.indexOf( '@rootpath' ) === 0 ) {
  		...
  		while ( context.isRoot && context._view.component ) {
  			context = context._view.component.parentFragment.findContext();
  		}

  		var match$1 = keypathExpr.exec( ref );
  		if ( match$1 && match$1[1] ) {
  			var model$1 = resolveReference( fragment, match$1[1] );
  			if ( model$1 ) return model$1.getKeypathModel( fragment._view.root );
  		}
  		return context.getKeypathModel( fragment._view.root );
  	}
  	...
  	if ( ref === '@this' ) {
  		return fragment._view.viewmodel.getViewModel();
  	}
  	if ( ref === '@global' ) {
  	...
  	// ancestor references
  	if ( ref[0] === '~' ) return fragment._view.viewmodel.joinAll( splitKeypathI( ref.slice( 2 ) ) );
  	if ( ref[0] === '.' ) {
  		var parts = ref.split( '/' );
  ...
  function $View$get ( keypath, opts ) {
  ...
  // sections when you do e.g. `_view.splice( 'items', 2, 2 );
  ...
  function merge$1 ( _view, model, array, options ) {
  	var promise = runloop.start( _view, true );
  	var value = model.get();
  ...
  function update$2 ( _view, model ) {
  	...
  	updateHook.fire( _view, model );

  	return promise;
  }

  function $View$update ( keypath ) {
  ...
  function resolve$1 ( path, _view ) {
  	var ref = findModel( this, path ), model = ref.model, instance = ref.instance;
  	return model ? model.getKeypath( _view || instance ) : path;
  }
  ...
  function findModel ( el, path ) {
  	...
  	return { model: resolveReference( frag, path ), instance: frag._view };
  }
  ...
  function add$1 ( keypath, value ) {
  	...
  	return set( this._view, build$1( this, keypath, value ).map( function ( pair ) {
  ...
  function merge ( keypath, array, options ) {
  	return merge$1( this._view, findModel( this, keypath ).model, array, options );
  }
  ...
  function set$1 ( keypath, value ) {
  	return set( this._view, build$1( this, keypath, value ) );
  }
  ...
  function toggle ( keypath ) {
  	...
  	return set( this._view, [ [ model, !model.get() ] ] );
  }
  ...
  function update$1 ( keypath ) {
  	return update$2( this._view, findModel( this, keypath ).model );
  }
  ...
  function getBindingPath ( _view ) {
  	var ref = getBindingModel( this ), model = ref.model, instance = ref.instance;
  	if ( model ) return model.getKeypath( _view || instance );
  }
  ...
  function getBindingModel ( ctx ) {
  	var el = ctx._element;
  	return { model: el.binding && el.binding.model, instance: el.parentFragment._view };
  }

  function setBinding ( value ) {
  	var ref = getBindingModel( this ), model = ref.model;
  	return set( this._view, [ [ model, value ] ] );
  }
  ...
  function rootpath () {
  	warnOnceIfDebug( ("Object property rootpath is deprecated, please use resolve( _view.root ) instead.") );
  	return this.resolve( this._view.root );
  }
  ...
  function addHelpers ( obj, element ) {
  	defineProperties( obj, {
  		...
  		_view: { value: element.parentFragment._view },
  ...
  function staticInfo( node ) {
  	...
  	if ( !node || !node.__view ) return {};

  	var storage = node.__view;
  ...
  function $View$insert ( target, anchor ) {
  	if ( !this.fragment.rendered ) {
  		// TODO create, and link to, documentation explaining this
  		throw new Error( 'The API has changed - you must call `_view.render(target[, anchor])` to render your $View instance. Once rendered you can use `_view.insert()`.' );
  	}
  	...
  	( target.___view_instances__ || ( target.___view_instances__ = [] ) ).push( this );
  	this.isDetached = false;

  	fireInsertHook( this );
  }

  function fireInsertHook( _view ) {
  	insertHook.fire( _view );

  	_view.findAllComponents('*').forEach( function ( child ) {
  ...
  function observe ( keypath, callback, options ) {
  	...
  	// add observers to the $View instance, so they can be
  	// cancelled on _view.teardown()
  ...
  function createObserver ( _view, keypath, callback, options ) {
  	var viewmodel = _view.viewmodel;
  	...
  	if ( !~wildcardIndex ) {
  		...
  		if ( key !== '' && !viewmodel.has( key ) ) {
  			...
  			if ( _view.component && !_view.isolated ) {
  				model = resolveReference( _view.component.parentFragment, key );
  				if ( model ) {
  		...
  		return new Observer( _view, model, callback, options );
  	}
  	...
  	return new PatternObserver( _view, baseModel, keys.splice( wildcardIndex ), callback, options );
  }

  var Observer = function Observer ( _view, model, callback, options ) {
  	var this$1 = this;

  		this.context = options.context || _view;
  	this.callback = callback;
  	this._view = _view;
  	...
  	if ( model ) this.resolved( model );
  		...
  		this.resolver = new ReferenceResolver( _view.fragment, options.keypath, function ( model ) {
  ...
  Observer.prototype.resolved = function resolved ( model ) {
  	this.model = model;
  	this.keypath = model.getKeypath( this._view );
  ...
  var PatternObserver = function PatternObserver ( _view, baseModel, keys, callback, options ) {
  	var this$1 = this;
  	...
  	var baseKeypath = baseModel.getKeypath( _view );
  	...
  	models.forEach( function ( model ) {
  		this$1.newValues[ model.getKeypath( this$1._view ) ] = model.get();
  	});
  ...
  PatternObserver.prototype.shuffle = function shuffle ( newIndices ) {
  	...
  	var base = this.baseModel.getKeypath( this._view );
  	...
  	var this$1 = this;
  		...
  		if ( !this.changed.length ) {
  			this.baseModel.findMatches( this.keys ).forEach( function ( model ) {
  				var keypath = model.getKeypath( this$1._view );
  			...
  			var ok = this.baseModel.isRoot ?
  				...
  				this.changed.map( function ( keys ) { return this$1.baseModel.getKeypath( this$1._view ) + '.' + keys.map( escapeKey ).join( '.' ); } );
  ...
  function observeList ( keypath, callback, options ) {
  	if ( typeof keypath !== 'string' ) {
  		throw new Error( '_view.observeList() must be passed a string as its first argument' );
  	}

  	var model = this.viewmodel.joinAll( splitKeypathI( keypath ) );
  	var observer = new ListObserver( this, model, callback, options || {} );

  	// add observer to the $View instance, so it can be
  	// cancelled on _view.teardown()
  ...
  function $View$off ( eventName, callback ) {
  ...
  function $View$on ( eventName, callback ) {
  ...
  function $View$once ( eventName, handler ) {
  ...
  var PREFIX = '/* Debuhg Info component styles */';
  ...
  function render$1 ( _view, target, anchor, occupants ) {
  	// if `noIntro` is `true`, temporarily disable transitions
  	var transitionsEnabled = _view.transitionsEnabled;
  	if ( _view.noIntro ) _view.transitionsEnabled = false;

  	var promise = runloop.start( _view, true );
  	runloop.scheduleTask( function () { return renderHook.fire( _view ); }, true );

  	if ( _view.fragment.rendered ) {
  		throw new Error( 'You cannot call _view.render() on an already rendered instance! Call _view.unrender() first' );
  	}

  	anchor = getElement( anchor ) || _view.anchor;

  	_view.el = target;
  	_view.anchor = anchor;

  	// ensure encapsulated CSS is up-to-date
  	if ( _view.cssId ) applyCSS();

  	if ( target ) {
  		( target.___view_instances__ || ( target.___view_instances__ = [] ) ).push( _view );

  		if ( anchor ) {
  			var docFrag = doc.createDocumentFragment();
  			_view.fragment.render( docFrag );
  			target.insertBefore( docFrag, anchor );
  		} else {
  			_view.fragment.render( target, occupants );
  		}
  	}

  	runloop.end();
  	_view.transitionsEnabled = transitionsEnabled;

  	return promise.then( function () { return completeHook.fire( _view ); } );
  }

  function $View$render ( target, anchor ) {
  	if ( this.torndown ) {
  		warnIfDebug( '_view.render() was called on a $View instance that was already torn down' );
  	...
  	if ( !this.append && target ) {
  		...
  		var others = target.___view_instances__;
  ...
  var dataRvcGuidPattern = /\[data-_view-css~="\{[a-z0-9-]+\}"]/g;
  ...
  function transformCss ( css, id ) {
  	var dataAttr = "[data-_view-css~=\"{" + id + "}\"]";
  ...
  function validate ( data ) {
  	...
  	if ( data && data.constructor !== Object ) {
  		if ( typeof data === 'function' ) {
  			// TODO do we need to support this in the new $View() case?
  ...
  var dataConfigurator = {
  			...
  			for ( key in options.data ) {
  				...
  				if ( value && typeof value === 'object' ) {
  					if ( isObject( value ) || isArray( value ) ) {
  						warnIfDebug( ("Passing a `data` option with object and array properties to $View.extend() is discouraged, as mutating them is likely to cause bugs. Consider using a data function instead:\n\n  // this...\n  data: function () {\n    return {\n      myObject: {}\n    };\n  })\n\n  // instead of this:\n  data: {\n    myObject: {}\n  }") );
  					}
  				}
  			}
  		}
  	...
  	init: function ( Parent, _view, options ) {
  		var result = combine$1( Parent.prototype.data, options.data );

  		if ( typeof result === 'function' ) result = result.call( _view );

  		// bind functions to the _view instance at the top level,
  		// unless it's a non-POJO (in which case alarm bells should ring)
  		if ( result && result.constructor === Object ) {
  			for ( var prop in result ) {
  				if ( typeof result[ prop ] === 'function' ) result[ prop ] = bind( result[ prop ], _view );
  			}
  		}
  	...
  	reset: function ( _view ) {
  		var result = this.init( _view.constructor, _view, _view.viewmodel );
  		_view.viewmodel.root.set( result );
  		return true;
  	}
  };
  ...
  function combine$1 ( parentValue, childValue ) {
  	...
  	// the default data object on $View or a component.
  	// then _view.set() ends up setting on the prototype!
  ...
  function fromComputationString ( str, bindTo ) {
  	...
  	var functionBody = 'return (' + str.replace( pattern, function ( match, keypath ) {
  		hasThis = true;
  		return ("___view.get(\"" + keypath + "\")");
  	}) + ');';

  	if ( hasThis ) functionBody = "var ___view = this; " + functionBody;
  ...
  function readReference ( parser ) {
  	...
  	// bug out if it's a keyword (exception for ancestor/restricted refs - see https://github.com/_viewjs/_view/issues/1497)
  ...
  function processDirective ( tokens, parentParser, type ) {
  	...
  	if ( typeof tokens === 'string' ) {
  		...
  		if ( type === EVENT && ( match = methodCallPattern.exec( tokens ) ) ) {
  			warnIfDebug( parentParser.getContextMessage( pos, ("Unqualified method events are deprecated. Prefix methods with '@this.' to call methods on the current $View instance.") )[2] );
  ...
  // See https://github.com/_viewjs/template-spec for information
  // about the $View template specification
  ...
  var TEMPLATE_INSTRUCTIONS = "Either preparse or use a _view runtime source that includes the parser. ";

  var COMPUTATION_INSTRUCTIONS = "Either use:\n\n\t$View.parse.computedStrings( component.computed )\n\nat build time to pre-convert the strings to functions, or use functions instead of strings in computed properties.";


  function throwNoParse ( method, error, instructions ) {
  	if ( !method ) {
  		fatal( ("Missing $View.parse - cannot parse " + error + ". " + instructions) );
  	}
  }
  ...
  var parser = {
  	...
  	fromId: function ( id, options ) {
  		if ( !doc ) {
  			...
  			throw new Error( ("Cannot retrieve template #" + id + " as $View is not running in a browser.") );
  		}
  	...
  	getParseOptions: function ( _view ) {
  		// Could be $View or a Component
  		if ( _view.defaults ) { _view = _view.defaults; }
  	...
  	parseFor: function( template, _view ) {
  		return this.parse( template, this.getParseOptions( _view ) );
  	}
  };
  ...
  var templateConfigurator = {
  	...
  	init: function ( Parent, _view, options ) {
  		...
  		// _view.template, and not bother passing through the Parent object.
  		...
  		if ( typeof template === 'function' ) {
  			var fn = template;
  			template = getDynamicTemplate( _view, fn );

  			_view._config.template = {
  				fn: fn,
  				result: template
  			};
  		}

  		template = parseTemplate( template, _view );

  		// TODO the naming of this is confusing - _view.template refers to [...],
  		// but Component.prototype.template refers to {v:1,t:[],p:[]}...
  		// it's unnecessary, because the developer never needs to access
  		// _view.template
  		_view.template = template.t;

  		if ( template.p ) {
  			extendPartials( _view.partials, template.p );
  		}
  	},

  	reset: function ( _view ) {
  		var result = resetValue( _view );

  		if ( result ) {
  			var parsed = parseTemplate( result, _view );

  			_view.template = parsed.t;
  			extendPartials( _view.partials, parsed.p, true );
  ...
  function resetValue ( _view ) {
  	var initial = _view._config.template;
  	...
  	var result = getDynamicTemplate( _view, initial.fn );
  ...
  function getDynamicTemplate ( _view, fn ) {
  	return fn.call( _view, {
  		fromId: parser.fromId,
  		isParsed: parser.isParsed,
  		parse: function ( template, options ) {
  			if ( options === void 0 ) options = parser.getParseOptions( _view );
  ...
  function parseTemplate ( template, _view ) {
  	if ( typeof template === 'string' ) {
  		// parse will validate and add expression functions
  		template = parseAsString( template, _view );
  	}
  	else {
  ...
  function parseAsString ( template, _view ) {
  	// ID of an element containing the template?
  	if ( template[0] === '#' ) {
  		template = parser.fromId( template );
  	}

  	return parser.parseFor( template, _view );
  }
  ...
  function validate$1( template ) {
  	...
  	// Check we're using the correct version
  		...
  		throw new Error( ("Mismatched template version (expected " + TEMPLATE_VERSION + ", got " + (template.v) + ") Please ensure you are using the latest version of Debuhg Info in your build process as well as in your app") );
  	}
  }
  ...
  Registry.prototype.reset = function reset ( _view ) {
  	var registry = _view[ this.name ];
  	var changed = false;
  ...
  var config = {
  	...
  	init: function ( Parent, _view, options ) { return configure( 'init', Parent, _view, options ); },

  	reset: function ( _view ) {
  		return order.filter( function ( c ) {
  			return c.reset && c.reset( _view );
  	...
  	for ( var key in options ) {
  		if ( isStandardKey.hasOwnProperty( key ) ) {
  			...
  			if ( key !== 'el' && typeof value === 'function' ) {
  				warnIfDebug( ("" + key + " is a $View option that does not expect a function and will be ignored"),
  ...
  var Item = function Item ( options ) {
  	...
  	this._view = options.parentFragment._view;
  ...
  var ExpressionProxy = (function (Model) {
  	function ExpressionProxy ( fragment, template ) {
  		...
  		Model.call( this, fragment._view.viewmodel, null );
  	...
  	ExpressionProxy.prototype.getValue = function getValue () {
  		...
  		try {
  			...
  			result = this.fn.apply( this.fragment._view, params );
  		} catch ( err ) {
  ...
  var ReferenceExpressionChild = (function (Model) {
  	...
  	ReferenceExpressionChild.prototype.retrieve = function retrieve () {
  		...
  		return parent && parent[ this.key ] ? parent[ this.key ] : undefined;
  	};
  ...
  var ReferenceExpressionProxy = (function (Model) {
  	function ReferenceExpressionProxy ( fragment, template ) {
  		...
  		this.dirty = true;
  		this.root = fragment._view.viewmodel;
  		this.template = template;
  ...
  function updateId ( reset ) {
  	...
  	if ( this._view.nodes[ node.id ] === node ) delete this._view.nodes[ node.id ];
  	if ( reset ) return node.removeAttribute( 'id' );

  	this._view.nodes[ value ] = node;
  ...
  function updateMultipleSelectValue ( reset ) {
  	...
  	if ( reset ) {
  		...
  		while ( i-- ) {
  			...
  			var optionValue = option.__view ?
  				option.__view.value :
  				option.value; // options inserted via a triple don't have __view
  ...
  function updateRadioValue ( reset ) {
  	...
  	node.value = this.node.__view.value = value;
  ...
  function updateValue ( reset ) {
  	if ( !this.locked ) {
  		if ( reset ) {
  			...
  			this.node.value = this.node.__view.value = null;
  			return;
  		}

  		var value = this.getValue();

  		this.node.value = this.node.__view.value = value;
  ...
  function updateRadioName ( reset ) {
  	...
  	else this.node.checked = ( this.getValue() == this.node.__view.value );
  }
  ...
  var Attribute = (function (Item) {
  	function Attribute ( options ) {
  		...
  		this._view = this.parentFragment._view;
  ...
  mutatorMethods.forEach( function ( methodName ) {
  	var method = function () {
  		...
  		this.__view.wrappers.forEach( function ( r ) { if ( r.magic ) r.magic.locked = true; } );
  		...
  		this.__view.setting = true;
  		var i = this.__view.wrappers.length;
  		while ( i-- ) {
  			processWrapper( this$1.__view.wrappers[i], this$1, methodName, newIndices );
  		}

  		runloop.end();

  		this.__view.setting = false;

  		// unlock the magic arrays... magic... bah
  		this.__view.wrappers.forEach( function ( r ) { if ( r.magic ) r.magic.locked = false; } );
  ...
  var arrayAdaptor = {
  	filter: function ( object ) {
  		...
  		return isArray( object ) && ( !object.__view || !object.__view.setting );
  	},
  	wrap: function ( _view, array, keypath ) {
  		return new ArrayWrapper( _view, array, keypath );
  	}
  };

  var ArrayWrapper = function ArrayWrapper ( _view, array ) {
  	this.root = _view;
  	this.value = array;
  	...
  	if ( !array.__view ) {
  		// define a non-enumerable __view property to store the wrappers
  		defineProperty( array, '__view', {
  			value: {
  				wrappers: [],
  				instances: [],
  				setting: false
  	...
  	// store the _view instance, so we can handle transitions later
  	if ( !array.__view.instances[ _view._guid ] ) {
  		array.__view.instances[ _view._guid ] = 0;
  		array.__view.instances.push( _view );
  	}

  	array.__view.instances[ _view._guid ] += 1;
  	array.__view.wrappers.push( this );
  };
  ...
  ArrayWrapper.prototype.teardown = function teardown () {
  	...
  	if ( !wrappers.length ) {
  		...
  		// remove _view instance if possible
  ...
  try {
  	...
  	magicAdaptor = {
  		...
  		wrap: function ( _view, value, keypath ) {
  			return new MagicWrapper( _view, value, keypath );
  		}
  	};
  } catch ( err ) {
  	magicAdaptor = false;
  ...
  function createOrWrapDescriptor ( originalDescriptor, _view, keypath, wrapper ) {
  	if ( originalDescriptor.set && originalDescriptor.set.__magic ) {
  		originalDescriptor.set.__magic.dependants.push({ _view: _view, keypath: keypath });
  		return originalDescriptor;
  	}

  	var setting;

  	var dependants = [{ _view: _view, keypath: keypath }];
  ...
  function revert ( descriptor, _view, keypath ) {
  	...
  	while ( i-- ) {
  		...
  		if ( dependant._view === _view && dependant.keypath === keypath ) {
  ...
  var MagicWrapper = function MagicWrapper ( _view, value, keypath ) {
  	var this$1 = this;
  	...
  	Object.keys( value ).forEach( function ( key ) {
  		...
  		var descriptor = createOrWrapDescriptor( originalDescriptor, _view, childKeypath, this$1 );
  ...
  var MagicArrayWrapper = function MagicArrayWrapper ( _view, array, keypath ) {
  	this.value = array;

  	this.magic = true;

  	this.magicWrapper = magicAdaptor$1.wrap( _view, array, keypath );
  	this.arrayWrapper = arrayAdaptor.wrap( _view, array, keypath );
  ...
  var magicArrayAdaptor = {
  	filter: function ( object, keypath, _view ) {
  		return magicAdaptor$1.filter( object, keypath, _view ) && arrayAdaptor.filter( object );
  	},

  	wrap: function ( _view, array, keypath ) {
  		return new MagicArrayWrapper( _view, array, keypath );
  	}
  };
  ...
  var ViewModel = (function (Model) {
  	...
  	ViewModel.prototype = Object.create( Model && Model.prototype );
  	ViewModel.prototype.constructor = ViewModel;

  	ViewModel.prototype.getKeypath = function getKeypath() {
  		return '@this';
  	};
  ...
  var RootModel = (function (Model) {
  	function RootModel ( options ) {
  		...
  		this.isRoot = true;
  		this.root = this;
  		this._view = options._view; // TODO sever this link

  		this.value = options.data;
  		this.adaptors = options.adapt;
  		this.adapt();

  		this.computationContext = options._view;
  		this.computations = {};
  	...
  	RootModel.prototype.getViewModel = function getViewModel() {
  		return this._viewModel || ( this._viewModel = new ViewModel( this._view ) );
  	};
  	...
  	RootModel.prototype.joinKey = function joinKey ( key, opts ) {
  		...
  		if ( key === '@this' ) return this.getViewModel();
  ...
  function getComputationSignature ( _view, key, signature ) {
  	var getter;
  	var setter;
  	...
  	if ( typeof signature === 'function' ) {
  		getter = bind( signature, _view );
  		getterString = signature.toString();
  		getterUseStack = true;
  	}

  	if ( typeof signature === 'string' ) {
  		getter = createFunctionFromString( signature, _view );
  		getterString = signature;
  	}

  	if ( typeof signature === 'object' ) {
  		if ( typeof signature.get === 'string' ) {
  			getter = createFunctionFromString( signature.get, _view );
  			getterString = signature.get;
  		} else if ( typeof signature.get === 'function' ) {
  			getter = bind( signature.get, _view );
  		...
  		if ( typeof signature.set === 'function' ) {
  			setter = bind( signature.set, _view );
  ...
  function construct ( _view, options ) {
  	if ( $View.DEBUG ) welcome();

  	initialiseProperties( _view );

  	// TODO remove this, eventually
  	defineProperty( _view, 'data', { get: deprecateViewData });

  	// TODO don't allow `onconstruct` with `new $View()`, there's no need for it
  	constructHook.fire( _view, options );

  	// Add registries
  	registryNames$1.forEach( function ( name ) {
  		_view[ name ] = extendObj( create( _view.constructor[ name ] || null ), options[ name ] );
  	});

  	// Create a viewmodel
  	var viewmodel = new RootModel({
  		adapt: getAdaptors( _view, _view.adapt, options ),
  		data: dataConfigurator.init( _view.constructor, _view, options ),
  		_view: _view
  	});
  	...
  	var computed = extendObj( create( _view.constructor.prototype.computed ), options.computed );

  	for ( var key in computed ) {
  		var signature = getComputationSignature( _view, key, computed[ key ] );
  ...
  function getAdaptors ( _view, protoAdapt, options ) {
  	protoAdapt = protoAdapt.map( lookup );
  	var adapt = ensureArray( options.adapt ).map( lookup );

  	adapt = combine$2( protoAdapt, adapt );

  	var magic = 'magic' in options ? options.magic : _view.magic;
  	var modifyArrays = 'modifyArrays' in options ? options.modifyArrays : _view.modifyArrays;
  	...
  	function lookup ( adaptor ) {
  		if ( typeof adaptor === 'string' ) {
  			adaptor = findInViewHierarchy( 'adaptors', _view, adaptor );
  ...
  function initialiseProperties ( _view ) {
  	// Generate a unique identifier, for places where you'd use a weak map if it
  	// existed
  	_view._guid = 'r-' + uid++;

  	// events
  	_view._subs = create( null );
  	...
  	if(!_view.component){
  		_view.root = _view;
  		_view.parent = _view.container = null; // TODO container still applicable?
  	}

  }

  function deprecateViewData () {
  	throw new Error( 'Using `_view.data` is no longer supported - you must use the `_view.get()` API instead' );
  }

  function getChildQueue ( queue, _view ) {
  	return queue[ _view._guid ] || ( queue[ _view._guid ] = [] );
  }

  function fire ( hookQueue, _view ) {
  	var childQueue = getChildQueue( hookQueue.queue, _view );

  	hookQueue.hook.fire( _view );
  	...
  	delete hookQueue.queue[ _view._guid ];
  }
  ...
  HookQueue.prototype.begin = function begin ( _view ) {
  	this.inProcess[ _view._guid ] = true;
  };

  HookQueue.prototype.end = function end ( _view ) {
  	...
  	// elsewise, handoff to parent to fire when ready
  		...
  		getChildQueue( this.queue, parent ).push( _view );
  	}

  	delete this.inProcess[ _view._guid ];
  };

  var configHook = new Hook( 'config' );
  var initHook = new HookQueue( 'init' );

  function initialise ( _view, userOptions, options ) {
  	Object.keys( _view.viewmodel.computations ).forEach( function ( key ) {
  		var computation = _view.viewmodel.computations[ key ];

  		if ( _view.viewmodel.value.hasOwnProperty( key ) ) {
  			computation.set( _view.viewmodel.value[ key ] );
  		}
  	});

  	// init config from Parent and options
  	config.init( _view.constructor, _view, userOptions );

  	configHook.fire( _view );
  	initHook.begin( _view );
  	...
  	if ( _view.template ) {
  		...
  		if ( options.cssIds || _view.cssId ) {
  			cssIds = options.cssIds ? options.cssIds.slice() : [];

  			if ( _view.cssId ) {
  				cssIds.push( _view.cssId );
  			}
  		}

  		_view.fragment = fragment = new Fragment({
  			owner: _view,
  			template: _view.template,
  			cssIds: cssIds
  		}).bind( _view.viewmodel );
  	}
  	...
  	if ( fragment ) {
  		...
  		var el = getElement( _view.el );
  		if ( el ) {
  			var promise = _view.render( el, _view.append );

  			if ( $View.DEBUG_PROMISES ) {
  				promise['catch']( function ( err ) {
  					warnOnceIfDebug( 'Promise debugging is enabled, to help solve errors that happen asynchronously. Some browsers will log unhandled promise rejections, in which case you can safely disable promise debugging:\n  $View.DEBUG_PROMISES = false;' );
  					warnIfDebug( 'An error happened during rendering', { _view: _view });
  					logIfDebug( err );
  ...
  var ViewEvent = function ViewEvent ( _view, name ) {
  	this._view = _view;
  	this.name = name;
  	this.handler = null;
  };

  ViewEvent.prototype.listen = function listen ( directive ) {
  	var _view = this._view;

  	this.handler = _view.on( this.name, function () {
  		var event;
  ...
  ViewEvent.prototype.unlisten = function unlisten () {
  	this.handler.cancel();
  };
  ...
  var EventDirective = function EventDirective ( options ) {
  	...
  	if ( this.element.type === COMPONENT ) {
  		this.template.n.split( '-' ).forEach( function ( n ) {
  			this$1.events.push( new ViewEvent( this$1.element.instance, n ) );
  		});
  	} else {
  		this.template.n.split( '-' ).forEach( function ( n ) {
  			var fn = findInViewHierarchy( 'events', this$1._view, n );
  ...
  EventDirective.prototype.fire = function fire ( event, passedArgs ) {
  	...
  	if ( this.fn ) {
  		...
  		var _view = this._view;
  		var oldEvent = _view.event;

  		_view.event = event;
  		var result = this.fn.apply( _view, values ).pop();
  ...
  var Component = (function (Item) {
  	function Component ( options, ComponentConstructor ) {
  		...
  		instance.parent = this.parentFragment._view;
  		...
  		// for any _view instance, but for now, just
  		// for components and just for _view...
  	...
  	Component.prototype.setupEvents = function setupEvents () {
  		...
  		Object.keys( this.template.v ).forEach( function ( key ) {
  			...
  			eventNames.forEach( function ( eventName ) {
  				var event = new ViewEvent( this$1.instance, eventName );
  	...
  	Component.prototype.unbind = function unbind$1 () {
  		...
  		if ( instance.fragment.rendered && instance.el.___view_instances__ ) {
  			removeFromArray( instance.el.___view_instances__, instance );
  		}
  ...
  Decorator.prototype.render = function render () {
  	var this$1 = this;
  		...
  		runloop.scheduleTask( function () {
  		var fn = findInViewHierarchy( 'decorators', this$1._view, this$1.name );
  		...
  		if ( this$1.argsFn ) {
  			...
  			args = this$1.argsFn.apply( this$1._view, args );
  		} else {
  			args = this$1.dynamicArgs ? this$1.argsFragment.getArgsList() : this$1.args;
  		}

  		this$1.intermediary = fn.apply( this$1._view, [ this$1.node ].concat( args ) );
  		...
  		if ( nameChanged || !this.intermediary.update ) {
  			...
  			if ( this.dynamicArgs ) {
  				if ( this.argsFragment.dirty ) {
  					...
  					this.intermediary.update.apply( this._view, args );
  				}
  			}
  			else if ( this.argsFn ) {
  				...
  				this.intermediary.update.apply( this._view, this.argsFn.apply( this._view, args$1 ) );
  			}
  			else {
  				this.intermediary.update.apply( this._view, this.args );
  			}
  		}
  	}
  ...
  function warnAboutAmbiguity ( description, _view ) {
  	warnOnceIfDebug( ("The " + description + " being used for two-way binding is ambiguous, and may cause unexpected results. Consider initialising your data to eliminate the ambiguity"), { _view: _view });
  }
  ...
  var Binding = function Binding ( element, name ) {
  	...
  	if ( !model ) {
  		...
  		warnAboutAmbiguity( ("'" + (interpolator.template.r) + "' reference"), this._view );
  	}

  	else if ( model.isUnresolved ) {
  		// reference expressions (e.g. foo[bar])
  		model.forceResolution();
  		warnAboutAmbiguity( 'expression', this._view );
  	}

  	// TODO include index/key/keypath refs as read-only
  	else if ( model.isReadonly ) {
  		var keypath = model.getKeypath().replace( /^@/, '' );
  			warnOnceIfDebug( ("Cannot use two-way binding on <" + (element.name) + "> element: " + keypath + " is read-only. To suppress this warning use <" + (element.name) + " twoway='false'...>"), { _view: this._view });
  		return false;
  	}
  ...
  function handleDomEvent () {
  	this.__view.binding.handleChange();
  }
  ...
  var CheckboxNameBinding = (function (Binding) {
  	function CheckboxNameBinding ( element ) {
  		...
  		this.checkboxName = true; // so that _view.updateModel() knows what to do with this
  ...
  function handleBlur () {
  	...
  	var value = this.__view.binding.model.get();
  ...
  function handleDelay ( delay ) {
  	...
  	return function () {
  		...
  		timeout = setTimeout( function () {
  			var binding = this$1.__view.binding;
  ...
  var MultipleSelectBinding = (function (Binding) {
  	...
  	MultipleSelectBinding.prototype.getValue = function getValue () {
  		...
  		for ( var i = 0; i < len; i += 1 ) {
  			...
  			if ( option.selected ) {
  				var optionValue = option.__view ? option.__view.value : option.value;
  	...
  	MultipleSelectBinding.prototype.setFromNode = function setFromNode ( node ) {
  		...
  		while ( i-- ) {
  			...
  			result[i] = option.__view ? option.__view.value : option.value;
  		}
  ...
  var RadioBinding = (function (Binding) {
  	function RadioBinding ( element ) {
  		...
  		this.siblings = getSiblings( this._view._guid + this.element.getAttribute( 'name' ) );
  ...
  var SingleSelectBinding = (function (Binding) {
  	...
  	SingleSelectBinding.prototype.getValue = function getValue () {
  		...
  		for ( i = 0; i < len; i += 1 ) {
  			...
  			if ( options[i].selected && !options[i].disabled ) {
  				return option.__view ? option.__view.value : option.value;
  			}
  		}
  	};
  	...
  	SingleSelectBinding.prototype.setFromNode = function setFromNode ( node ) {
  		...
  		this.model.set( option.__view ? option.__view.value : option.value );
  	};
  	...
  	if ( element.name === 'input' ) {
  		...
  		if ( type === 'radio' || type === 'checkbox' ) {
  			...
  			if ( bindName && bindChecked ) {
  				if ( type === 'radio' ) {
  					warnIfDebug( 'A radio input can have two-way binding on its name attribute, or its checked attribute - not both', { _view: element.root });
  				} else {
  					// A checkbox with bindings for both name and checked - see https://github.com/_viewjs/_view/issues/1749
  					return CheckboxBinding;
  				}
  			}
  ...
  var Element = (function (Item) {
  	...
  	Element.prototype.createTwowayBinding = function createTwowayBinding () {
  		var shouldBind = 'twoway' in this ? this.twoway : this._view.twoway;
  	...
  	Element.prototype.render = function render$1 ( target, occupants ) {
  		...
  		// a data-_view-css attribute, for CSS encapsulation
  		if ( this.parentFragment.cssIds ) {
  			node.setAttribute( 'data-_view-css', this.parentFragment.cssIds.map( function ( x ) { return ("{" + x + "}"); } ).join( ' ' ) );
  		}
  		...
  		if ( this._introTransition && this._view.transitionsEnabled ) {
  	...
  	Element.prototype.unrender = function unrender ( shouldDestroy ) {
  		...
  		// outro transition
  		if ( this._outroTransition && this._view.transitionsEnabled ) {
  ...
  function getNamespace ( element ) {
  	...
  	return element._view.el.namespaceURI;
  }
  ...
  function createMapping ( item ) {
  	...
  	if ( template.length === 1 && template[0].t === INTERPOLATOR ) {
  		...
  		if ( !item.model ) {
  			warnOnceIfDebug( ("The " + (item.name) + "='{{" + (template[0].r) + "}}' mapping is ambiguous, and may cause unexpected results. Consider initialising your data to eliminate the ambiguity"), { _view: item.element.instance }); // TODO add docs page explaining item
  			item.parentFragment._view.get( item.name ); // side-effect: create mappings as necessary
  ...
  var Option = (function (Element) {
  	...
  	Option.prototype.render = function render ( target, occupants ) {
  		...
  		if ( !this.attributeByName.value ) {
  			this.node.__view.value = this.getAttribute( 'value' );
  		}
  	};
  ...
  function getPartialTemplate ( _view, name, parentFragment ) {
  	// If the partial in instance or view heirarchy instances, great
  	var partial = getPartialFromRegistry( _view, name, parentFragment || {} );
  	if ( partial ) return partial;

  	// Does it exist on the page as a script tag?
  	partial = parser.fromId( name, { noThrow: true } );
  	if ( partial ) {
  		// parse and register to this _view instance
  		var parsed = parser.parseFor( partial, _view );

  		// register extra partials on the _view instance if they don't already exist
  		if ( parsed.p ) fillGaps( _view.partials, parsed.p );

  		// register (and return main partial if there are others in the template)
  		return _view.partials[ name ] = parsed.t;
  	}
  }

  function getPartialFromRegistry ( _view, name, parentFragment ) {
  	// if there was an instance up-hierarchy, cool
  	var partial = findParentPartial( name, parentFragment.owner );
  	if ( partial ) return partial;

  	// find first instance in the _view or view hierarchy that has this partial
  	var instance = findInstance( 'partials', _view, name );
  	...
  	if ( typeof partial === 'function' ) {
  		...
  		partial = fn.call( _view, parser );
  	}

  	if ( !partial && partial !== '' ) {
  		warnIfDebug( noRegistryFunctionReturn, name, 'partial', 'partial', { _view: _view });
  		return;
  	}
  	...
  	if ( !parser.isParsed( partial ) ) {
  		// use the parseOptions of the _view instance on which it was found
  		var parsed = parser.parseFor( partial, instance );

  		// Partials cannot contain nested partials!
  		// TODO add a test for this
  		if ( parsed.p ) {
  			warnIfDebug( 'Partials ({{>%s}}) cannot contain nested inline partials', name, { _view: _view });
  		}
  ...
  function findOwner ( _view, key ) {
  	return _view.partials.hasOwnProperty( key )
  		? _view
  		: findConstructor( _view.constructor, key);
  }
  ...
  var Partial = (function (Mustache) {
  	...
  	Partial.prototype.bind = function bind () {
  		...
  		var template = this.refName ? getPartialTemplate( this._view, this.refName, this.parentFragment ) || null : null;
  		var templateObj;
  		...
  		if ( !template ) {
  			...
  			if ( this.model && ( templateObj = this.model.get() ) && typeof templateObj === 'object' && ( typeof templateObj.template === 'string' || isArray( templateObj.t ) ) ) {
  				if ( templateObj.template ) {
  					...
  					templateObj = parsePartial( this.template.r, templateObj.template, this._view );
  				} else {
  	...
  	Partial.prototype.forceResetTemplate = function forceResetTemplate () {
  		...
  		if ( this.refName ) {
  			this.partialTemplate = getPartialTemplate( this._view, this.refName, this.parentFragment );
  		}

  		// then look for the resolved name
  		if ( !this.partialTemplate ) {
  			this.partialTemplate = getPartialTemplate( this._view, this.name, this.parentFragment );
  		}
  	...
  	Partial.prototype.setTemplate = function setTemplate ( name, template ) {
  		...
  		if ( !template && template !== null ) template = getPartialTemplate( this._view, name, this.parentFragment );
  				...
  				if ( template && typeof template === 'string' && template !== this.name ) {
  					...
  					if ( template.t !== this.source && template.template !== this.source ) {
  						if ( template.template ) {
  							...
  							template = parsePartial( this.name, template.template, this._view );
  						} else {
  ...
  function parsePartial( name, partial, _view ) {
  	var parsed;

  	try {
  		parsed = parser.parse( partial, parser.getParseOptions( _view ) );
  	} catch (e) {
  ...
  RepeatedFragment.prototype.updatePostShuffle = function updatePostShuffle () {
  	...
  	// if the array was spliced outside of _view, sometimes there are leftover fragments not in the newIndices
  ...
  var Select = (function (Element) {
  	...
  	Select.prototype.sync = function sync () {
  		...
  		if ( selectValue !== undefined ) {
  			...
  			options.forEach( function ( o ) {
  				var optionValue = o.__view ? o.__view.value : o.value;
  		...
  		setTimeout( function () {
  			...
  			function checkComplete () {
  				if ( jsTransitionsComplete && cssTransitionsComplete ) {
  					...
  					t._view.fire( t.name + ':end', t.node, t.isIntro );
  					resolve();
  				}
  			}
  			...
  			setTimeout( function () {
  				...
  				while ( i-- ) {
  					...
  					if ( !CSS_TRANSITIONS_ENABLED || cannotUseCssTransitions[ hash ] ) {
  						...
  						if ( index === -1 ) {
  							warnIfDebug( 'Something very strange happened with transitions. Please raise an issue at https://github.com/_viewjs/_view/issues - thanks!', { node: t.node });
  						} else {
  							changedProperties.splice( index, 1 );
  						}
  				...
  				if ( propertiesToTransitionInJs.length ) {
  					...
  					if ( typeof options.easing === 'string' ) {
  						easing = t._view.easing[ options.easing ];
  ...
  Transition.prototype.animateStyle = function animateStyle ( style, value, options ) {
  	...
  	if ( !options ) {
  		warnOnceIfDebug( 'The "%s" transition does not supply an options object to `t.animateStyle()`. This will break in a future version of $View. For more info see https://github.com/ViewJS/$View/issues/340', this.name );
  		options = this;
  	}
  ...
  Transition.prototype.bind = function bind () {
  	...
  	if ( typeof this.name === 'function' ) {
  		...
  		this._fn = findInViewHierarchy( 'transitions', _view, this.name );
  	}

  	if ( !this._fn ) {
  		warnOnceIfDebug( missingPlugin( this.name, 'transition' ), { _view: _view });
  	}
  ...
  Transition.prototype.start = function start () {
  	...
  	if ( this.argsFn ) {
  		...
  		args = this.argsFn.apply( this._view, values );
  	}

  	var promise = this._fn.apply( this._view, [ this ].concat( args ) );
  ...
  var Yielder = (function (Item) {
  	function Yielder ( options ) {
  		...
  		this.container = options.parentFragment._view;
  	...
  	Yielder.prototype.bind = function bind () {
  		...
  		if ( !template ) {
  			warnIfDebug( ("Could not find template for partial \"" + name + "\""), { _view: this._view });
  			template = [];
  		}

  		this.fragment = new Fragment({
  			owner: this,
  			_view: this.container.parent,
  			template: template
  		}).bind();
  	};
  	...
  	Yielder.prototype.setTemplate = function setTemplate ( name ) {
  		var template = this.parentFragment._view.partials[ name ];
  ...
  function getComponentConstructor ( _view, name ) {
  	var instance = findInstance( 'components', _view, name );
  	var Component;

  	if ( instance ) {
  		Component = instance.components[ name ];

  		// best test we have for not $View.extend
  		if ( !Component._Parent ) {
  			...
  			if ( !Component ) {
  				warnIfDebug( noRegistryFunctionReturn, name, 'component', 'component', { _view: _view });
  				return;
  			}

  			if ( typeof Component === 'string' ) {
  				// allow string lookup
  				Component = getComponentConstructor( _view, Component );
  			}
  ...
  function createItem ( options ) {
  	...
  	if ( options.template.t === ELEMENT ) {
  		...
  		var ComponentConstructor = getComponentConstructor( options.parentFragment._view, options.template.e );
  ...
  var Fragment = function Fragment ( options ) {
  	...
  	this._view = options._view || ( this.isRoot ? options.owner : this.parent._view );

  	this.componentParent = ( this.isRoot && this._view.component ) ? this._view.component.parentFragment : null;
  ...
  Fragment.prototype.findContext = function findContext () {
  	...
  	if ( !fragment ) return this._view.viewmodel;
  ...
  Fragment.prototype.findNextNode = function findNextNode ( item ) {
  	...
  	if ( this.isRoot ) {
  		if ( this._view.component ) {
  			return this._view.component.parentFragment.findNextNode( this._view.component );
  		}

  		// TODO possible edge case with other content
  		// appended to this._view.el?
  		return null;
  	}
  ...
  Fragment.prototype.findParentNode = function findParentNode () {
  	...
  	do {
  		...
  		if ( fragment.isRoot && !fragment._view.component ) { // TODO encapsulate check
  			return fragment._view.el;
  		}
  ...
  Fragment.prototype.getArgsList = function getArgsList () {
  	if ( this.dirtyArgs ) {
  		var values = {};
  		var source = processItems( this.items, values, this._view._guid );
  ...
  function $View$resetTemplate ( template ) {
  ...
  function $View$set ( keypath, value ) {
  	var _view = this;

  	return set( _view, build( _view, keypath, value ) );
  }
  ...
  function $View$subtract ( keypath, d ) {
  	return add( this, keypath, ( d === undefined ? -1 : -d ) );
  }
  ...
  function $View$teardown () {
  	if ( this.torndown ) {
  		warnIfDebug( '_view.teardown() was called on a $View instance that was already torn down' );
  	...
  	if ( this.fragment.rendered && this.el.___view_instances__ ) {
  		removeFromArray( this.el.___view_instances__, this );
  	}
  ...
  function $View$toHTML () {
  	return this.fragment.toString( true );
  }

  function $View$transition ( name, node, params ) {
  ...
  function $View$unrender () {
  	if ( !this.fragment.rendered ) {
  		warnIfDebug( '_view.unrender() was called on a $View instance that was not rendered' );
  		return Promise$1.resolve();
  	}
  ...
  function $View$updateModel ( keypath, cascade ) {
  	var promise = runloop.start( this, true );
  ...
  var proto = {
  	...
  	detach: $View$detach,
  	find: $View$find,
  	findAll: $View$findAll,
  	findAllComponents: $View$findAllComponents,
  	findComponent: $View$findComponent,
  	findContainer: $View$findContainer,
  	findParent: $View$findParent,
  	fire: $View$fire,
  	get: $View$get,
  	getNodeInfo: getNodeInfo,
  	insert: $View$insert,
  	link: link$1,
  	merge: this$View$merge,
  	observe: observe,
  	...
  	resetTemplate: $View$resetTemplate,
  	reverse: reverse$1,
  	set: $View$set,
  	shift: shift$1,
  	sort: sort$1,
  	splice: splice$1,
  	subtract: $View$subtract,
  	...
  	unlink: unlink$1,
  	unrender: $View$unrender,
  	unshift: unshift$1,
  	update: $View$update,
  	updateModel: $View$updateModel
  };
  ...
  function extendOne ( Parent, options ) {
  	...
  	// if we're extending with another $View instance...
  	//
  	//   var Human = $View.extend(...), Spider = $View.extend(...);
  	//   var Spiderman = Human.extend( Spider );
  	//
  	// ...inherit prototype methods and default options as well
  	if ( options.prototype instanceof $View ) {
  ...
  // Debuhg Info makes liberal use of things like Array.prototype.indexOf. In
  // older browsers, these are made available via a shim - here, we do a quick
  // pre-flight check to make sure that either a) we're not in a shit browser,
  // or b) we're using a $View-legacy.js build
  ...
  if (
  	...
  	throw new Error( 'It looks like you\'re attempting to use Debuhg Info in an older browser. You\'ll need to use one of the \'legacy builds\' in order to continue - see http://docs._viewjs.org/latest/legacy-builds for more information.' );
  }
  ...
  extendObj( $View.prototype, proto, defaults );
  $View.prototype.constructor = $View;

  // alias prototype as `defaults`
  $View.defaults = $View.prototype;
